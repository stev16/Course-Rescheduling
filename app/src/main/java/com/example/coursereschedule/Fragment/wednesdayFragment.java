...
public class wednesdayFragment extends Fragment {
    private RecyclerView subjectViewWednesday;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View wednesday = inflater.inflate(R.layout.fragment_wednesday, container, false);

        subjectViewWednesday=wednesday.findViewById(R.id.subjectViewWednesday);
        subjectViewWednesday.setLayoutManager(new LinearLayoutManager(wednesday.getContext()));
        subjectModelList= new ArrayList<>();
        subjectAdapter = new subjectAdapter(wednesday.getContext(), subjectModelList);
        subjectViewWednesday.setAdapter(subjectAdapter);

        auth = FirebaseAuth.getInstance();

        Query db= FirebaseDatabase.getInstance().getReference().child("Timetable").child(auth.getCurrentUser().getUid())
                .child("Subjects").orderByChild("subjectDay").equalTo("Wednesday");
