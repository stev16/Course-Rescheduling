...
public class tuesdayFragment extends Fragment {
    ...
    private RecyclerView subjectViewTuesday;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View tuesday= inflater.inflate(R.layout.fragment_tuesday, container, false);

        subjectViewTuesday=tuesday.findViewById(R.id.subjectViewTuesday);
        subjectViewTuesday.setLayoutManager(new LinearLayoutManager(tuesday.getContext()));
        subjectModelList= new ArrayList<>();
        subjectAdapter = new subjectAdapter(tuesday.getContext(), subjectModelList);
        subjectViewTuesday.setAdapter(subjectAdapter);

        auth =FirebaseAuth.getInstance();

        Query db= FirebaseDatabase.getInstance().getReference().child("Timetable").child(auth.getCurrentUser().getUid())
                .child("Subjects").orderByChild("subjectDay").equalTo("Tuesday");
